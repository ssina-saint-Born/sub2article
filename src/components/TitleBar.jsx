...
/**
 * TitleBar — the top-of-window app bar.
 *
 * Always renders the brand block (logo + name + version). The three
 * window-control buttons (minimize / maximize / close) render only in
 * the Electron desktop build: in the web build those actions have no
 * real OS window to act on, so they are omitted entirely rather than
 * shown as dead affordances.
 */
export default function TitleBar() {
  const showControls = isElectron();
  ...
  return (
    <div className="drag-region flex items-center justify-between h-10 px-4 bg-surface-950 border-b border-surface-800/50 shrink-0">
      ...
      {/* Right: Window controls — Electron only (omitted in Web Mode) */}
      {showControls && (
